...
@implementation KPUArrayOfTicketSubStateInfo
...
- (id) initWithXml: (DDXMLElement*) __node __request:(KPURequestResultHandler*) __request {
    if(self = [self init])
    {
        NSArray* tempItems = [__node elementsForName:@"TicketSubStateInfo"];
        for(DDXMLElement* child in tempItems)
        {
            [self.items addObject:(KPUTicketSubStateInfo*)[__request createObject:child type:[KPUTicketSubStateInfo class]]];
        }
...
{
    for (KPUTicketSubStateInfo* item in self.items) {
        DDXMLElement* propertyElement=[__request writeElement:item type:[KPUTicketSubStateInfo class] name:@"TicketSubStateInfo" URI:@"http://www.apoyar.net/" parent:__parent skipNullProperty:NO];
...
- (void)removeObjectIdenticalTo:(KPUTicketSubStateInfo*)anObject inRange:(NSRange)range {
	return [self.items removeObjectIdenticalTo:anObject inRange:range];
}

- (void)removeObjectIdenticalTo:(KPUTicketSubStateInfo*)anObject {
...
+ (id)array {
	return [[ KPUArrayOfTicketSubStateInfo alloc] init];
}

+ (id)arrayWithObject:(KPUTicketSubStateInfo*)anObject {
	return [[ KPUArrayOfTicketSubStateInfo alloc] initWithObjects:anObject, nil];
}

+ (id)arrayWithObjects:(const KPUTicketSubStateInfo* *)objects count:(NSUInteger)cnt {
	return [[ KPUArrayOfTicketSubStateInfo alloc] initWithObjects:objects count:cnt];
}

+ (id)arrayWithObjects:(KPUTicketSubStateInfo*)firstObj, ... {
	KPUArrayOfTicketSubStateInfo* a = [ KPUArrayOfTicketSubStateInfo array];
	id eachObject;
...
- (id)initWithObjects:(KPUTicketSubStateInfo*)firstObj, ... {
	if(self = [self init]) {
		id eachObject;
		va_list argumentList;
		if (firstObj) {
...
+ (id)arrayWithArray:(NSArray *)array {
	return [[ KPUArrayOfTicketSubStateInfo alloc] initWithArray:array];
}

+ (id)arrayWithContentsOfFile:(NSString *)path {
	return [[ KPUArrayOfTicketSubStateInfo alloc] initWithContentsOfFile:path];
}

+ (id)arrayWithContentsOfURL:(NSURL *)url {
	return [[ KPUArrayOfTicketSubStateInfo alloc] initWithContentsOfURL:url];
}

+ (id)arrayWithCapacity:(NSUInteger)numItems {
	return [[ KPUArrayOfTicketSubStateInfo alloc] initWithCapacity:numItems];
}
